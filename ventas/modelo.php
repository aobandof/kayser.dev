    ...
    if($_POST['opcion']=="ventas") {
        ...
        if( $registros === false ){
            ...
            While ($reg = sqlsrv_fetch_array( $registros, SQLSRV_FETCH_NUMERIC)) {
                $cont++;
                $cuerpo.="<tr class='fila'><td class='col-xs-1'>$cont</td><td class='col-xs-7'>".$reg[0]."</td><td class='col-xs-4'>".number_format ( floatval($reg[1]), 0 , ',', '.')."</td></tr>";
                $venta_total+=$reg[1];
            }
            $venta_total=number_format ( floatval($venta_total), 0 , ',', '.');
            $pie.="<td colspan='2' class='col-xs-8'>VENTA TOTAL :&nbsp</td><td class='col-xs-4'>".$venta_total."</td>";
