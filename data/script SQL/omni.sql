...
select t0.IdAlmacen,t0.IdArticulo,CAST(SUM(t0.Cantidad) AS int) as cant from   Existencia as t0
...
select t0.IdArticulo, t0.Cantidad, CAST(SUM(CAST(t0.Cantidad) AS int)-30 AS int) as Cant from   Existencia as t0 inner join Ubicacion as t1 on t0.IdUbicacion=t1.IdUbicacion where  t0.IdAlmacen = '01' AND t0.IdUbicacion LIKE '01%' and t1.Nivel in ('1','2') GROUP BY IdArticulo,Cantidad HAVING SUM(Cantidad)>30 ORDER BY IdArticulo
