...
if($_POST['opcion']=="cargar_selects_dependientes") {
  ...
  if($tablas_sku[$padre]['dep']=='padre')
    $codigo_padre=getIdFromName($_POST['nom_tabla_padre'],$_POST['val_tabla_padre']);
  else
    $codigo_padre=$_POST['val_tabla_padre'];
  foreach ($tablas_sku as $tabla => $array_tabla) { // recorremos todo el array con las tablas, campos y relaciones
    $ops="";//"<option value=''></option>";
    if(isset($array_tabla['dep'])){
      if($_POST['nom_tabla_padre']==$array_tabla['dep']){
        addGrandChild($tabla); // buscamos descendientes dependientes de esta tabla y la agregamos al array
        $nombre_id=$array_tabla['id'];
        $nombre_name=$array_tabla['campo'];
        if($array_tabla['bd']=="mysql"){ // la tabla y la relacion estan en MOTOR MYSQL
          ...
          // echo $query;
          if(!$registros=$mysqli->query($query)){
            $error="Error: La ejecucion de la consulta fallo debidoo a: \nQuery: " . $query . "\nErrno: " . $mysqli->errno . "\nError: " . $mysqli->error . "\n";
            $options[]=array('error'=> $error);
            continue;//pasamos al siguiente recorrido de foreach
          }else {
            $ops="";
          ...
          if(!$registros=$mysqli->query($query_relacion)){
            ...
            while($reg=$registros->fetch_array()){
              ...
              $ops.="<option value='".(string)$reg[1]."'>".(string)$array_tabla_extraida["$nom_id_dep_rel"]."</option>";
            }
          }
        }
        $options[]=array('tabla'=>$tabla, 'options'=>$ops);
